    ...
    async setup() {
        api.addEventListener("int_bridge_session", (event) => {
            ...
            if (editWidget && value !== undefined) {
                setupWidgetSync(node, editWidget, "/int_bridge/sync", "edited_value");
            ...
            if (!passthrough) {
                ...
                showBridgeModal(
                    node,
                    node.title,
                    { inputType: "number", value: value, step: 1 },
                    (editedValue) => {
                        disableButtons(node);
                        ...
                        api.fetchApi("/int_bridge/confirm", {
                            method: "POST",
        ...
        api.addEventListener("int_bridge_resume", (event) => {
